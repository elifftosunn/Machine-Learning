...
def processedDataCsvCreate(mongodf):
    ...
    for col in NaN_Columns:
        missingValue(mongodf).categoric_Freq(col)
    NaN_Columns,missingDf = dataUnderstand(mongodf).missingValueTables()
    print("NaN_Columns: ",NaN_Columns,"\n",missingDf)
...
def separatingColon(df, col, split, sep1, sep2): # columns =,:,"," separating
...
# NaN_Columns,missingDf = dataUnderstand(df).missingValueTables()
# print("NaN_Columns: ",NaN_Columns,"\n",missingDf)
# df.to_csv("datas/processedDatas/secondTenThousandProcessData.csv",index=False)
